...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (cached) {
      ...
      memoryCacheSet(CACHE_KEY, [tempProduct], DEFAULT_CACHE_TTL);
    }
    ...
    if (variant === 'simple') {
      setImmediate(async () => {
        try {
          ...
          const current = memoryCacheGet<any[]>(CACHE_KEY);
          if (current) {
    ...
    return NextResponse.json({
      ...
      message: variant === 'simple'
        ? `Product "${tempProduct.title}" created using Write-Behind (simple)`
        : `Product "${tempProduct.title}" queued using Write-Behind (queue)`,
    });
  } catch (error) {
    const responseTimeMs = Date.now() - startTime;
    return NextResponse.json(
  ...
  return NextResponse.json({
    ...
    variants: {
      simple: {
        ...
        flow: 'Client → API → Cache → setImmediate(DB) → Response',
      ...
      queue: {
        ...
        flow: 'Client → API → Cache → Queue → Batch → DB → Response',
        useCase: 'High-volume writes where batching improves throughput',
      },
    },
    endpoint: 'POST /api/products/write-behind?variant=simple|queue',
    body: {
